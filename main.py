...
# TODO: FEATURE: Add a scoring system that calculates a score based on the amount of time that is needed to pass the level. Should also account for the amount and types of extras that you collect. Score should be visible at all times and should get shown again when a level has been completed.
# TODO: FEATURE: Add a highscore system so that players can record their best runs.
# TODO: FEATURE: Add a difficulty setting that will influence game speed, block health and power up distribution.
# TODO: FEATURE: The level number should be clearly visible when playing any given level. This could get handled with the title of the window where the game is running, or it could display a text in the background of the level.
